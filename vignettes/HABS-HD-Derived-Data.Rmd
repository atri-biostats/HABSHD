...
HD_longitudinal <- lapply(list(HD_Visit_1, HD_Visit_2, HD_Visit_3), 
  ...
  group_by(Med_ID) %>%
  mutate(
    `Age at baseline` = min(Age, na.rm=TRUE),
    `Years from baseline` = Age - `Age at baseline`) %>%
  ungroup() %>%
  select(Visit_ID, Med_ID, `Age at baseline`, `Years from baseline`,
    everything())
```
...
# Check for duplicated `Med_ID/Visit_ID` combinations in `HD_longitudinal`

```{r}
if(with(HD_longitudinal, any(duplicated(paste(Med_ID, Visit_ID))))){

  dups <- HD_longitudinal %>%
  ...
  dups %>%
    knitr::kable(caption = 'Duplicated Med_ID/Visit_ID found in HD_longitudinal. Duplicates will be removed after filling any missing values by carrying observations backward within Visit_ID.') %>%
    print()
  
  HD_longitudinal_dups <- HD_longitudinal %>%
    filter(paste(Med_ID, Visit_ID) %in% with(dups, paste(Med_ID, Visit_ID))) %>%
    group_by(Med_ID, Visit_ID) %>%
    tidyr::fill(names(.), .direction='up') %>%
    distinct(Med_ID, Visit_ID, .keep_all = TRUE) %>%
    ungroup()
  
  HD_longitudinal <- HD_longitudinal %>%
    filter(!paste(Med_ID, Visit_ID) %in% with(dups, paste(Med_ID, Visit_ID))) %>%
    bind_rows(HD_longitudinal_dups) %>%
    arrange(Med_ID, Visit_ID)
  
  if(with(HD_longitudinal, any(duplicated(paste(Med_ID, Visit_ID))))){
    stop("Duplicates Med_ID/Visit_ID in HD_longitudinal remain.")
  }
}else{
  message('No duplicates found.')
}
