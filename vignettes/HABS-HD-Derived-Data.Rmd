...
HD_MRI_ComBat <- lapply(mriVars, function(x){
  ...
  tmp1 <- tmp0 %>%
    rename(Y = matches(x)) %>%
    select(Med_ID, Visit_ID, MRI_Scanner, Y, Ethnicity, ID_Gender, scale_age) %>%
    arrange(Med_ID, Visit_ID) %>%
    na.omit()
  tmp1[, x] <- ComBatLong(Y ~  MRI_Scanner + (Ethnicity + ID_Gender)*scale_age,
    tmp1, 
...
HD_AB_FBB_ComBat <- lapply(fbbVars, function(x){
  ...
  tmp1 <- tmp0 %>%
    rename(Y = matches(x)) %>%
    select(Med_ID, Visit_ID, AB_FBB_Sanner, Y, Ethnicity, ID_Gender, scale_age) %>%
    arrange(Med_ID, Visit_ID) %>%
    na.omit()
  tmp1[, x] <- ComBatLong(Y ~  AB_FBB_Sanner + (Ethnicity + ID_Gender)*scale_age,
    tmp1, 
    random1 = ~ scale_age|Med_ID,
    random2 = ~ 1|Med_ID,
...
HD_Imaging_ComBat <- HD_Imaging %>%
  select(Visit_ID, Med_ID, Age, Ethnicity, ID_Education, ID_Gender, 
