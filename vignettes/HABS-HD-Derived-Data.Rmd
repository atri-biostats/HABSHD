...
# Derive `HD_Imaging_ComBat` harmonized data

MRI and FBB amyloid PET data are harmonized with respect to scanner type using ComBat for longitudinal data @beer2020longitudinal.

```{r mri-ComBat}
mriVars <- HD_Imaging %>% 
  select(`01_L_hippocampus`:`01_CorticalThickness_MetaROI`,
    `02_WMH_Volume_Log`, `02_WMH_Volume_Raw`, `04_Total_wmh`) %>%
  colnames()

tmp0 <- HD_Imaging %>%
  left_join(HD_Clinical %>% 
    arrange(Med_ID, Visit_ID) %>%
    group_by(Med_ID) %>%
    filter(!is.na(CDX_Cog)) %>%
    filter(!duplicated(Med_ID)) %>%
    select(Med_ID, CDX_Cog), by = 'Med_ID') %>%
  filter(!is.na(`01_MRI_Scanner`)) %>%
  mutate(
    scale_age = scale(Age)[,1],
    MRI_Scanner = as.factor(`01_MRI_Scanner`)
  )

HD_MRI_ComBat <- lapply(mriVars, function(x){
  message('ComBat: ', x)
  tmp1 <- tmp0 %>%
    rename(Y = matches(x)) %>%
    select(Med_ID, Visit_ID, MRI_Scanner, Y, CDX_Cog, ID_Gender, scale_age) %>%
    arrange(Med_ID, Visit_ID) %>%
    na.omit()
  tmp1[, x] <- ComBatLong(Y ~  MRI_Scanner + (CDX_Cog + ID_Gender)*scale_age,
    tmp1, 
    random1 = ~ scale_age|Med_ID,
    random2 = ~ 1|Med_ID,
    weights = varIdent(form = ~ 1 | MRI_Scanner))
  tmp1 %>% select(Med_ID, Visit_ID, matches(x))
}) %>% 
  reduce(full_join, by = c('Med_ID', 'Visit_ID'))
```

```{r AB-FBB-PET-ComBat}
fbbVars <- HD_Imaging %>% 
  select(`01_AB_FBB_AntPostCingulate_SUVR`:`01_AB_FBB_Global_SUVR`) %>%
  colnames()
...
tmp0 <- HD_Imaging %>%
  left_join(HD_Clinical %>% 
    ...
    filter(!duplicated(Med_ID)) %>%
    select(Med_ID, CDX_Cog), by = 'Med_ID') %>%
  filter(!is.na(`01_AB_FBB_Scanner`)) %>%
  mutate(
    scale_age = scale(Age)[,1],
    AB_FBB_Sanner = as.factor(`01_AB_FBB_Scanner`)
  )

HD_AB_FBB_ComBat <- lapply(fbbVars, function(x){
  message('ComBat: ', x)
  tmp1 <- tmp0 %>%
    rename(Y = matches(x)) %>%
    select(Med_ID, Visit_ID, AB_FBB_Sanner, Y, CDX_Cog, ID_Gender, scale_age) %>%
    arrange(Med_ID, Visit_ID) %>%
    na.omit()
  tmp1[, x] <- ComBatLong(Y ~  AB_FBB_Sanner + (CDX_Cog + ID_Gender)*scale_age,
    tmp1, 
    random1 = ~ scale_age|Med_ID,
    random2 = ~ 1|Med_ID,
    weights = varIdent(form = ~ 1 | AB_FBB_Sanner))
  tmp1 %>% select(Med_ID, Visit_ID, matches(x))
}) %>% 
  reduce(full_join, by = c('Med_ID', 'Visit_ID'))
```
...
```{r HD-Imaging-ComBat}
HD_Imaging_ComBat <- HD_Imaging %>%
  left_join(HD_subjinfo %>% select(Med_ID, CDX_Cog), by = 'Med_ID') %>%
  select(Visit_ID, Med_ID, Age, Ethnicity, CDX_Cog, ID_Education, ID_Gender, 
    `01_MRI_Scanner`, `01_AB_FBB_Scanner`) %>%
  full_join(HD_MRI_ComBat, by = c('Med_ID', 'Visit_ID')) %>%
  full_join(HD_AB_FBB_ComBat, by = c('Med_ID', 'Visit_ID')) %>%
  arrange(Med_ID, Visit_ID)
```
...
HD_subjinfo_imaging <- HD_Imaging_ComBat %>% 
  arrange(Med_ID, Visit_ID) %>%
  group_by(Med_ID) %>%
  mutate(Mean_ICV_HD = mean(`01_ICV_HD`, na.rm = TRUE)) %>%
  select(`Med_ID`, `Visit_ID`,
    `01_AB_FBB_AB_pos`, `01_TAU_PI2620_Medial_Temporal_SUVR`,
    `01_L_hippocampus`, `01_R_hippocampus`, `Mean_ICV_HD`, `02_WMH_Volume_Raw`,
    `02_WMH_Volume_Log`) %>%
  tidyr::fill(names(.), .direction='up') %>%
  filter(Visit_ID == 1) %>%
  ungroup()
